...
vel_img = ax[0].imshow(velocity_obs, vmin=0, vmax=np.nanmax(velocity_obs), cmap="magma", zorder=2)
...
vel_img = ax[1].imshow(velocity_iter, vmin=0, vmax=np.nanmax(velocity_obs), cmap="magma", zorder=2)
cbar = fig.colorbar(vel_img)
cbar.ax.set_ylabel('Surface Velocity (m a$^{-1}$)', rotation=90)
